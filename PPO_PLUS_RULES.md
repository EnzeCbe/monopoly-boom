...
- **Landing on an unowned property**: the player may buy it at list price if they can afford
  ...
  **auction** among all non-bankrupt players, starting with the player to the active
  player's left and continuing around the table.
...
- **Bankruptcy to a player creditor** (unpaid rent to another player): all of the bankrupt
  ...
  properties (unmortgaged, ownership simply transfers), and their Get-Out-of-Jail-Free card
  if they held one.
...
- **Sahipsiz bir mülke basmak**: oyuncu, gücü yetiyorsa liste fiyatından satın alabilir.
  Reddederse (veya gücü yetmezse) ve ardından sırayı bitirirse, mülk, aktif oyuncunun
  solundaki oyuncudan başlayarak ve masa etrafında devam ederek, iflas etmemiş tüm
  oyuncular arasında **açık artırmaya** çıkarılır.
...
- **İpotek verme**: geliştirilmemiş (0 evli), tamamen sahip olunan bir mülk, listelenen
  ...
  kazandırmaz. **İpoteği kaldırmak, ipotek değerinin %10 faiziyle birlikte geri ödenmesini
  gerektirir** (`cost = ipotek_değeri × 1.1`, bkz. `env.py` içindeki `_mortgage_actions`) —
  geleneksel faiz ücreti burada da alınır.
...
- **Bir oyuncu alacaklıya iflas** (başka bir oyuncuya ödenmemiş kira): iflas eden
  ...
  alır**: iflas eden oyuncunun kalan tüm nakdi, tüm mülkleri (ipoteksiz, sahiplik sadece
  devredilir) ve varsa Hapisten Çıkış Kartı.
